...
contract(Ethbay,([deployer, patient])=>{
    let ethbay;
    before(async () =>{
    ...
    describe('Deployment', async()=>{
        ...
        it('The deployed smart contract has the correct name', async()=>{
            const name = await ethbay.hospitalName();
            assert.equal(name, 'EECE571G HOSPITAL')
        })
    })
    ...
    describe('booking appointment', async()=>{
        ...
        before(async ()=>{
            result = await ethbay.bookAppointment('Dinesh Pabbi',23, web3.utils.toWei('1', 'Ether'),{from: patient,value: web3.utils.toWei('1', 'Ether')});
            totalNumber = await ethbay.appointCount();
        })
        it ('Creating item should be successful if all correct', async ()=>{
            //SUCCESSFUL
            assert.equal(totalNumber,1);
            const event = result.logs[0].args;
            assert.equal(event.id.toNumber(), totalNumber.toNumber(), 'item id is correct');
            assert.equal(event.patientName, 'Dinesh Pabbi','Patient name is correct');
            assert.equal(event.age.toNumber(),23,"Age is Correct");
            assert.equal(event.fee, '1000000000000000000','item price is correct');
        ...
        it ('Creating item should be failed if either no name or no price', async ()=>{
            ...
            await ethbay.bookAppointment('', web3.utils.toWei('1','Ether'), {from: patient}).should.be.rejected;
            //Price must be greater than 0
            await ethbay.bookAppointment('T-Shirt', web3.utils.toWei('0','Ether'), {from: patient}).should.be.rejected;
        })

        it ('Check the item created', async ()=>{
            const item = await ethbay.Appointments(patient);
            assert.equal(item.id.toNumber(), totalNumber.toNumber(), 'item id is correct');
            assert.equal(item.patientName, 'Dinesh Pabbi','Patient name is correct');
            assert.equal(item.age.toNumber(),23,"Age is Correct");
            assert.equal(item.fee, '1000000000000000000','item price is correct');
        })
